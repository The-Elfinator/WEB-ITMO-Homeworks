    ...
    beforeCreate() {
        ...
        this.$root.$on("onRegister", (login, username) => {
            ...
            if (login.length < 3 || login.length > 16) {
                this.$root.$emit("onRegisterValidationError", "Login should contain at least 3 and at most 16 symbols");
                return;
            }
            if (!/^([a-z]+)$/.test(login)) {
            ...
            if (username.length > 32) {
                this.$root.$emit("onRegisterValidationError", "Name should have at most 32 symbols");
                return;
            }
            if (/^(\s*)$/.test(username)) {
                this.$root.$emit("onRegisterValidationError", "Name shouldn't contain only whitespaces!");
                return;
            }
            ...
            if (users.length !== 0) {
                ...
                const id = Math.max(0, Math.max(...Object.keys(this.users))) + 1;
